...
# requires https://github.com/tatianapassali/artificial-disfluency-generation
    ...
    for fn_model in glob.glob(opt.model_path):
        ...
        if use_lard:
            for i, r in enumerate(r_list):
                if random() < 0.1106:
                    lard_output = lard.create_replacements(" ".join(r.tgt))
                    if lard_output[0]:
                        r.tgt = lard_output[2]
                        r.tgt_tags = (" ".join(lard_output[3]).replace("D", "I").replace("F", "O")).split()
                        r_list[i] = r
